...
struct JuliaParams {
  ...
  double centerX;
  double centerY;
  double juliaX;  // Parte real do parâmetro c
  double juliaY;  // Parte imaginária do parâmetro c
  int maxIterations;
  int width;
  int height;
};

__global__ void juliaKernel(uchar4* output, JuliaParams params) {
  ...
  // Ponto inicial z0 = x + yi
  double zx = params.centerX + (x - params.width/2.0) * scale;
  double zy = params.centerY + (y - params.height/2.0) * scale;
  ...
  for (iter = 0; iter < params.maxIterations; iter++) {
      ...
      // f(z) = z² + c, onde c é o parâmetro do conjunto de Julia
      double temp = zx2 - zy2 + params.juliaX;
      zy = 2.0 * zx * zy + params.juliaY;
      zx = temp;
  }
...
struct {
  ...
  .centerX = 0.0,
  .centerY = 0.0,
  .juliaX = -0.4,  // Valor inicial para c
  .juliaY = 0.6,   // Valor inicial para c
  .maxIterations = 100,
  .isDragging = false,
  .shouldUpdate = true
};
  ...
  if (action == GLFW_PRESS || action == GLFW_REPEAT) {
      ...
      switch (key) {
          ...
          case GLFW_KEY_DOWN:
              ...
              break;
          // Controles para o parâmetro c do conjunto de Julia
          case GLFW_KEY_LEFT:
              state.juliaX -= step;
              printf("c = %.3f + %.3fi\n", state.juliaX, state.juliaY);
              state.shouldUpdate = true;
              break;
          case GLFW_KEY_RIGHT:
              state.juliaX += step;
...
void updateJulia(int width, int height) {
  JuliaParams params = {
      state.zoom,
      state.centerX,
      state.centerY,
  ...
  juliaKernel<<<numBlocks, blockSize>>>(resources.deviceBuffer, params);
